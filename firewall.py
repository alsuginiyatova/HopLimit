...
FIREWALL_ADDRESS = PROXY_ADDRESS = ('::1', 65009, 0, 0)
...
def agent_normalize(pkt):
    ip_header = IPv6(src=pkt['IPv6'].src, dst='::1', hlim=64)
    udp_header = UDP(sport=pkt['UDP'].sport, dport=SECOND_CLIENT_ADDRESS[1]) packet_cov = ip_header / udp_header / pkt['UDP'].payload
...
if __name__ == '__main__':
    ...
    parser.add_argument('defend_mode', default='normalize', const='normalize', nargs='?',
    choices=['normalize', 'fict'])
    args = parser.parse_args()
    if args.defend_mode == "normalize":
        callback = agent_normalize
    if args.defend_mode == "fict":
